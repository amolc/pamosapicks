app.controller(
  ...
  function ($scope, $http, $window, $location, config) {
    ...
    $scope.getSubscriberStocks = function () {
      ...
      $http
        ...
        .success(function (response, status, headers, config) {
          ...
          $scope.winratechart(response.chart_data)
    ...
    $scope.getUserPortfolioDetail = function (req, res) {
      ...
      $http
        .get(
          ($scope.url = config.baseurl + "stockdata/subscribedetail/"+portfolioId+"/?robot=ig"),
          urlconfig
        )
        .success(function (response, status, headers, config) {
...
$scope.getSubscriberTranscation = function () {
  ...
  $http
    .get(
      ($scope.url =
        config.baseurl + `stockdata/get-robot-transaction/?subscriber_id=${portfolioId}&robot=ig`),
      urlconfig
    )
    .success(function (response, status, headers, config) {
...
$scope.winratechart = function (chart_data) {
  ...
  $scope.accountSocket = function() {
    var customerId = localStorage.getItem("customerId");
    var socket = new WebSocket($scope.wssurl+`ws/getall-ig-data-position/${customerId}/`)
  ...
  $scope.closePosition = function (dealId, portfolio_id) {
    $scope.dealId =  dealId
    $scope.portfolioId =  portfolio_id
