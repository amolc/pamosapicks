      ...
      if (data.amount == "") {
        ...
        $scope.addWithdraw = function (withdrawform) {
            ...
            if ($scope.validateentry == 1){ 
                ...
                $http.post($scope.baseurl + variable.bankmodule + "/addWithdrawalView", $scope.data, $scope.config)
                    .success(function (response, status) {
        ...
        $scope.getWithdrawals = function (req, res) {
            ...
            $scope.getdepositsurl = $scope.baseurl + variable.bankmodule + "/getWithdrawalbyCustomerId" ;
