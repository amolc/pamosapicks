...
app.controller(
  ...
  function ($scope, $http, $window, $location, config,variable) {
    ...
    $scope.myportfolioId = params.get("myportfolioId");
    ...
    $scope.gohighlight = function (myportfolioId) {

      $scope.url = $scope.baseurl + "backtesting/updateUserPortfoliov2" ;
      $scope.data = {}
      $scope.data.myportfolioId = myportfolioId
      $scope.data.highlight = 1
      ...
      $http.post($scope.startbacktest ,$scope.data)
        ...
        console.log(data);
          var urlconfig = {
            ...
            let url =  $scope.baseurl + variable.modulename + "/getUserPortfolioDetail"
            $http
              .post(url, data, urlconfig)
