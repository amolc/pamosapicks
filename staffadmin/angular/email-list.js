app.controller('email-ctrl', function($scope, $http, $window, $location, $sce, $timeout, store, config) {
    ...
    $scope.getStockSignals = function () {
      ...
      $http
        .get(
          ($scope.url =
            config.baseurl + `stockdata/emaillist`),
          urlconfig
        )
        .success(function (response, status, headers, config) {
          ...
          for(var i=0;i<$scope.signals.length;i++){
            console.log("...",$scope.signals[i]['message'])
            
            $scope.signals[i]['message'] = $sce.trustAsHtml($scope.signals[i]['message']);
