  ...
  function ($scope, $http, $window, $location, config) {
    ...
    $scope.getSubscriberStocks = function () {
      ...
      $http
        .get(
          ($scope.url =
            config.baseurl + `stockdata/subscriberportfoliostocks?customerId=${customerId}&portfolioId=${portfolioId}&robot=demo&status=1`),
          urlconfig
        )
        .success(function (response, status, headers, config) {
    ...
    $scope.getSubscriberInactiveStocks = function () {
      ...
      $http
        .get(
          ($scope.url =
            config.baseurl + `stockdata/subscriberportfoliostocks?customerId=${customerId}&portfolioId=${portfolioId}&robot=demo&status=0`),
          urlconfig
        )
        .success(function (response, status, headers, config) {
    ...
    $scope.getSubscriberTranscation = function () {
      ...
      $http
        .get(
          ($scope.url =
            config.baseurl + `stockdata/get-robot-transaction/?subscriber_id=${portfolioId}&robot=demo`),
          urlconfig
        )
        .success(function (response, status, headers, config) {
    ...
    $scope.getUserPortfolioDetail = function (req, res) {
      ...
      $http
        .get(
          ($scope.url = config.baseurl + "stockdata/subscribedetail/"+portfolioId+"/?robot=demo"),
          urlconfig
        )
        .success(function (response, status, headers, config) {
          $scope.portfolioDetails = response.data;
          $scope.addigdata = response.data;
...
$scope.editstockSumbit = function (data) {
  ...
  data['percentage'] = percentage
    ...
    var urlconfig = {
      ...
      $http
        .patch( $scope.baseurl + "stockdata/add-stocks/?robot=demo", data, urlconfig)
        .success(function (data, status, headers, config) {
          $scope.edit = data.data ;
