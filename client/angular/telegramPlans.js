    ...
    $scope.getallplans = function() {
      ...
      $scope.imLoading = true
        ...
        var urlconfig = {
          ...
          $http
            .get( $scope.baseurl + "customer/getall-telegram-plans/"  , urlconfig)
      ...
      data['plan_id'] = $scope.plan_id
        ...
        $http
          ...
          .success(function (response, status, headers, config) {
            $scope.imLoading = false
            if (data.plann_id != 1){
              localStorage.setItem('telegramplan_id', data.plan_id)
              localStorage.setItem('qrPlan', 'tele')
              location.href = `qr-plan.html?id=${$scope.portfolio_id}`
            }
            else{
              location.href = `telegram-plans.html?id=${$scope.portfolio_id}`
            }
