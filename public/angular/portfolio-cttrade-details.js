app.controller(
  ...
  function ($scope, $http, $window, $location, config) {
    ...
    $scope.getSubscriberStocks = function () {
      ...
      $http
        .get(
          ($scope.url =
            config.baseurl + `stockdata/subscriberportfoliostocks?customerId=${customerId}&portfolioId=${portfolioId}&robot=cttrade&status=1`),
          urlconfig
        )
        .success(function (response, status, headers, config) {
    ...
    $scope.getSubscriberInactiveStocks = function () {
      ...
      $http
        .get(
          ($scope.url =
            config.baseurl + `stockdata/subscriberportfoliostocks?customerId=${customerId}&portfolioId=${portfolioId}&robot=cttrade&status=0`),
          urlconfig
        )
        .success(function (response, status, headers, config) {
    ...
    $scope.getSubscriberTranscation = function () {
      ...
      $http
        .get(
          ($scope.url =
            config.baseurl + `stockdata/get-robot-transaction/?subscriber_id=${portfolioId}&robot=cttrade`),
          urlconfig
        )
        .success(function (response, status, headers, config) {
  ...
  $scope.addAPIDetails = function (data) {
    ...
    $scope.onaddAPISubmit = function (data) {
      $scope.imLoading = true
        ...
        $http
          .put(
            ($scope.url =
              config.baseurl + `stockdata/put-details/${portfolioId}/?robot=cttrade`),
              data,
            urlconfig
          )
          .success(function (response, status, headers, config) {
    ...
    $scope.getUserPortfolioDetail = function (req, res) {
      ...
      $http
        .get(
          ($scope.url = config.baseurl + "stockdata/subscribedetail/"+portfolioId+"/?robot=cttrade"),
          urlconfig
        )
        .success(function (response, status, headers, config) {
...
$scope.sharePercentageChart = function(chart_data) {
  ...
  $scope.buyAndsellOpenPosistion = function(stockId,customerId, portfolioId, server, direction) {
    debugger;
    ...
    $http
      .post(
        ($scope.url =
          config.baseurl + `stockdata/post-ct-trading-views/?stock_id=${stockId}&portfolio_id=${portfolioId}`),
          data, urlconfig
      )
      .success(function (response, status, headers, config) {
        $scope.imLoading = false;
