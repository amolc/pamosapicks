...
app.controller(
  ...
  function ($scope, $http, $window, $location, config) {
    ...
    $scope.portfolioList = function (req, res) {
      ...
      $http
        .get($scope.baseurl + `stockdata/get-customer-portfolios/?email=${email}&robot=demo`)
    ...
    $scope.goToPortfolioPage = function (portfolioId) {
      var portfolioUrl = "portfolio-demo-transaction.html?id=" + portfolioId;
  ...
  $scope.deleteSubmit = function () {
    ...
    $http.delete(config.baseurl + `stockdata/delete-portfolio/?portfolio_id=${PortID}&robot=demo`)
        .success(function(res) {
            if (res.status == 'false') {
            } else {
                $scope.response = res.data;
