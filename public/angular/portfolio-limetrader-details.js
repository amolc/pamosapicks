app.controller(
  ...
  function ($scope, $http, $window, $location, config) {
    ...
    $scope.getSubscriberStocks = function () {
      ...
      $http
        .get(
          ($scope.url =
            config.baseurl + `stockdata/subscriberportfoliostocks?customerId=${customerId}&portfolioId=${portfolioId}&robot=limetrader&status=1`),
          urlconfig
        )
        .success(function (response, status, headers, config) {
    ...
    $scope.getSubscriberInactiveStocks = function () {
      ...
      $http
        .get(
          ($scope.url =
            config.baseurl + `stockdata/subscriberportfoliostocks?customerId=${customerId}&portfolioId=${portfolioId}&robot=limetrader&status=0`),
          urlconfig
        )
        .success(function (response, status, headers, config) {
    ...
    $scope.getSubscriberTranscation = function () {
      ...
      $http
        .get(
          ($scope.url =
            config.baseurl + `stockdata/get-robot-transaction/?subscriber_id=${portfolioId}&robot=limetrader`),
          urlconfig
        )
        .success(function (response, status, headers, config) {
  ...
  $scope.addAPIDetails = function (data) {
    ...
    $scope.onaddAPISubmit = function (data) {
      $scope.imLoading = true
        ...
        $http
          .put(
            ($scope.url =
              config.baseurl + `stockdata/put-details/${portfolioId}/?robot=limetrader`),
              data,
            urlconfig
          )
          .success(function (response, status, headers, config) {
    ...
    $scope.getUserPortfolioDetail = function (req, res) {
      ...
      $http
        .get(
          ($scope.url = config.baseurl + "stockdata/subscribedetail/"+portfolioId+"/?robot=limetrader"),
          urlconfig
        )
        .success(function (response, status, headers, config) {
